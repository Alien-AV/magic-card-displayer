...
object CardDecoder {
    ...
    fun parseTranscript(transcript: String, enableNormalization: Boolean): CardParseResult {
        ...
        val hasShrinkPhrase = tokens.zipWithNext().any { (first, second) ->
            first == "shrink" && second == "it"
        }
        return CardParseResult(
            rankValue = lastRank,
            suitName = lastSuit,
            hasTriggerWord = hasTrigger,
            hasShrinkPhrase = hasShrinkPhrase
        )
    }
